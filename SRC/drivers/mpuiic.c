...
{			
	...
	GPIOA->CRL&=0XFF00FFFF;//PA4/5 推挽输出
...
unsigned char MPU_I2C_ReadOneByte(unsigned char I2C_Addr,unsigned char addr)
{
	...
	//IIC_Stop();//产生一个停止条件	
	MPU_IIC_Start();
	MPU_IIC_Send_Byte((I2C_Addr << 1)+1); res++;          //进入接收模式			   
	MPU_IIC_Wait_Ack();
...
u8 MPU_IICreadBits(u8 dev,u8 reg,u8 bitStart,u8 length)
{
...
u8 MPU_IICreadBit(u8 dev, u8 reg, u8 bitNum){
    u8 b;
    if (MPU_IICreadByte(dev, reg, &b) != 0) {
    b >>=bitNum;
    return b;
